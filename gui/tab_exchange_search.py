...
from gui.exchange_search_components.results_display import ResultsDisplay
from gui.exchange_search_components.ui_builder import MailSearchUI

EXCHANGE_SEARCH_CONFIG_FILE = "exchange_search_config.json"


class ExchangeSearchTab(ttk.Frame):
    def __init__(self, parent):
        ...
        from gui.exchange_search_components.pdf_history_manager import PDFHistoryManager
    ...
    def show_pdf_history(self):
        ...
        try:
            from gui.exchange_search_components.pdf_history_display import PDFHistoryDisplayWindow
    ...
    def discover_folders(self):
        ...
        def _discover():
            try:
                ...
                # Add debug logging
                from tools.logger import log
                log("[FOLDER DISCOVERY] Starting Exchange folder discovery")
                
                account = self.connection.get_exchange_account()
                log(f"[FOLDER DISCOVERY] Got Exchange account: {account is not None}")
    ...
    def _load_saved_exclusions(self):
        ...
        try:
            if os.path.exists(EXCHANGE_SEARCH_CONFIG_FILE):
                with open(EXCHANGE_SEARCH_CONFIG_FILE, "r", encoding='utf-8') as f:
                    config = json.load(f)
    ...
    def save_mail_search_config(self, show_message=True):
        ...
        try:
            ...
            with open(EXCHANGE_SEARCH_CONFIG_FILE, "w", encoding='utf-8') as f:
